    ...
    <section className="border-[3px] border-ws-black bg-ws-white p-5 shadow-[4px_4px_0_0_var(--color-ws-black)]">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="self-start border-[2px] border-ws-black px-4 py-2 text-[12px] font-black uppercase tracking-[0.08em] hover:border-ws-accent hover:bg-ws-page disabled:cursor-not-allowed disabled:opacity-50"
        >
          {loading ? 'Refreshing' : 'Refresh'}
        </button>
      </div>

      <div className="mt-5 grid grid-cols-2 border-[2px] border-ws-black md:hidden" aria-label="Draft editor view">
      ...
      {loading && (
        <p className="mt-5 border border-ws-black/15 bg-ws-page px-4 py-3 text-[14px] text-ws-black/65">
      ...
      {!loading && !draft && (
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.78fr)]">
          ...
          <aside className={mobileTab === 'edit' ? 'hidden md:block' : ''}>
            <div className="sticky top-4 border-[2px] border-ws-black/20 bg-ws-page p-4">
              <p className="text-[12px] font-black uppercase tracking-[0.12em] text-ws-black/60">Reader preview</p>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.78fr)]">
          <div className={mobileTab === 'preview' ? 'hidden md:block' : ''}>
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-4 border-t-[2px] border-ws-black/20 pt-5">
                <AddArticleManually
                  password={password}
...
function EditorCategorySection({
  ...
  return (
    <section className="border border-ws-black/15">
...
function PreviewColumn({
  ...
  return (
    <div className="sticky top-4 border-[2px] border-ws-black/20 bg-ws-page p-4">
      <p className="text-[12px] font-black uppercase tracking-[0.12em] text-ws-black/60">Reader preview</p>
      <h3 className="mt-2 font-[family-name:var(--font-display)] text-[28px] font-black leading-none tracking-tight">
        AI Today
      </h3>
          ...
          {groupedArticles.map(([category, items]) => {
            ...
            return (
              <section key={`${category}-preview`}>
                <h4 className="border-b-[2px] border-ws-black pb-1 text-[13px] font-black uppercase tracking-[0.1em]">
                  {meta?.icon ? `${meta.icon} ` : ''}{category}
                </h4>
