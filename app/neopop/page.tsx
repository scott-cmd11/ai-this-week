...
export const metadata: Metadata = {
  title: 'AI This Week — NeoPop prototype',
  description: 'Preview of a NeoPop-styled homepage for AI This Week.',
}
...
export default async function NeoPopHomePage() {
  ...
  return (
    <>
      ...
      {/* Past issues — a grid of cards with staggered shadows */}
      {past.length > 0 && (
      ...
      {issues.length === 0 && (
        <NeoPopCard bg="white" interactive={false}>
          ...
          <p className="text-[15px] mt-2">Once you publish an issue from Notion it will appear here.</p>
        </NeoPopCard>
      )}

      {/* Prototype footer note */}
      <div className="mt-16 pt-8 border-t-[3px] border-neopop-black">
        <p className="text-[13px] uppercase font-bold tracking-wide">
          NeoPop prototype · <Link href="/" className="underline hover:no-underline">Compare with the original site →</Link>
        </p>
      </div>
    </>
  )
